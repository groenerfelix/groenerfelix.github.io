...
export function TimelineRow({ entry }: { entry: TimelineEntry }) {
  ...
  return (
    <article className="group grid gap-4 border-t border-border py-6 md:grid-cols-[180px_1fr] md:gap-8">
      <div className="flex min-h-full flex-row-reverse items-center justify-center self-stretch text-center text-sm text-primary md:flex-col">
        <span>{end}</span>
        <span className="mr-3 h-2 w-2 rounded-full bg-primary md:mt-3 md:mr-0" />
        <span className="relative mx-2 h-px max-w-10 flex-1 overflow-hidden bg-border md:mx-0 md:my-2 md:h-auto md:w-px">
        ...
        <span className="mr-3 md:mt-3 md:mr-0">{start}</span>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div className="pt-1 text-base text-muted-foreground">
              {entry.org}
            </div>
          </div>
        </div>
        ...
        <ul className="space-y-3 text-sm leading-7 text-muted-foreground">
          {entry.bullets.map((bullet, index) => (
            <li key={`${entry.id}-bullet-${index}`} className="mb-0 flex gap-2">
              <span className="mt-3 h-px w-2 shrink-0 bg-border" />
              <span>{bullet}</span>
            </li>
          ))}
          <li className="mb-0 flex gap-2">
            <span className="mt-3 h-px w-2 shrink-0 bg-border" />
